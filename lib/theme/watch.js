...
class Watch extends BaseClass {
    ...
    /**
     * @param {{skipStart:boolean|undefined},{port:number|undefined}} options
     * @return {Promise<null>}
     */
    async run(options = {}) {
        ...
        if(options.port){
            process.env.ASSETS_PORT = options.port;
        }
       ...
       //this.runTheme('serve');
        ...
        let assetsPort = options.port || process.env.ASSETS_PORT || ASSETS_PORT;
        exec(`salla theme serve --port=${assetsPort}`, { stdio: "inherit" });
