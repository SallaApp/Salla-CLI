...
class Watch extends ThemeCommand {
    ...
    async run(options = {}) {
        ...
        this.runThemeCommand(`push --token ${tokens.github.access_token} --name ${tokens.github.login}`);
        ...
        let themeConfig = await this._ConfigManager.read();
        themeConfig['draft_id'] = response.id;
    ...
    async createDraftTheme() {
        ...
        const {repo_url, themeName, themeId} = await this._ConfigManager.read();
    ...
    async createLocalServer() {
        ...
        return WebDevServer.Server.CreateNew()
            .SetDocumentRoot(this.path().join(this.constants().THEME_PATH, "assets"))
            .SetPort(assetsPort)
            .Start((success, err) => {
