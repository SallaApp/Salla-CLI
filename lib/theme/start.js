...
class Start extends ThemeCommand {
    ...
    async run(cliOptions = {}) {
        ...
        answers.themeName = answers.themeName.replace(/\s/g,'-'); //replace spaces to underscore
     ...
     */
    getDefaultThemeName() {
        return this._isEmptyDir ? this.path().basename(this.constants().THEME_PATH).replace(/\s/g,'-') : undefined;
    }
