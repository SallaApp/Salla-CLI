...
const authenticationPath = require("path").join(dir, "config.json");
...
class AuthenticationManager {
    constructor() {
        this._authPath = authenticationPath;
        this._fs = fsModule;
        this._fsUtils = fsUtilsModule;
        this._logger = console;
        this._folderName = dir;
        this.skip_tokens_check = false;//to be avoid re checking valid tokens
    }
    ...
    async tokens(withoutForce = false) {
        ...
        try {
            this.configData = await this._fsUtils.parseJsonFile(this._authPath);
    ...
    async save(token, withLog = true) {
        if (!await this._fs.existsSync(this._folderName)) {
            await this._fs.mkdirSync(this._folderName, {
                recursive: true,
            });
        }
        if (token.github) {
        ...
        await this._fs.writeFile(this._authPath, token, (err) => {
