...
class AuthManager extends BaseClass {
  ...
  async tokens(withoutForce = false) {
    ...
    try {
      ...
      this.error(
        `Failed to read: ${CLI_CONFIG_FILE}\n${error.name}:${error.message}`
      );
    }
    ...
    let authPort =
      this.commandName === "auth" && this.options.port
  ...
  async save(token, withLog = true) {
    ...
    return this.fileSys().writeFile(
      ...
      (err) => {
        ...
        if (withLog) {
          Logger.succ("Token stored to: " + CLI_CONFIG_FILE);
        }
      }
    );
  }
  ...
  async isGithubTokenValid() {
    try {
      ...
      let message =
        error && error.message ? `${error.name}: ${error.message}` : "Unknown!";
